...
contract FileAccessControl is Ownable, ReentrancyGuard {
     ...
     */
    function setUserAttributes(address user, bytes32[] calldata attributes) external {
        // Anyone may set their own attributes; only file owners or admin can set for others.
        require(
            msg.sender == user || msg.sender == owner(),
            "ABAC: not authorised to set attributes"
        );
    ...
    /**
     * @notice Explicitly grant access to a recipient (overrides policy check).
     * @param fileId                The file to share.
     * @param recipient             Address to grant access to.
     * @param recipientAttributes   Attributes to set for the recipient at the same time.
     */
    function grantAccess(
        ...
        if (recipientAttributes.length > 0) {
            _userAttributes[recipient] = recipientAttributes;
        }
    ...
    /**
     * @notice Check whether a user satisfies the file access policy OR has an explicit grant.
    ...
    function checkAccess(address user, uint256 fileId) external view returns (bool) {
        // Explicit grant takes priority
        if (_accessGrants[fileId][user]) return true;

        // File owner always has access
        if (_fileOwner[fileId] == user) return true;

        // Attribute-policy check (user must possess ALL required attributes)
        bytes32[] storage policy = _filePolicies[fileId];
        if (policy.length == 0) return false; // no policy set = no access
